...
if __name__ == '__main__':
    ...
    petr = Student('Тарасевич Петр Петрович', 'subjects.csv')
    kate = Student('Антохина Екатерина Владимировна', 'subjects.csv')

    # Добавляем оценки и результаты тестов для Петра
    petr.add_grade('Математика', 5)
    petr.add_grade('Математика', 5)
    petr.add_grade('Математика', 4)
    petr.add_grade('Физика', 4)
    petr.add_grade('Физика', 3)
    petr.add_test_score('Математика', 87)
    petr.add_test_score('Математика', 91)
    petr.add_test_score('Физика', 57)

    # Добавляем оценки и результаты тестов для Екатерины
    kate.add_grade('Математика', 5)
    kate.add_grade('Математика', 5)
    kate.add_test_score('Математика', 65)
    kate.add_test_score('Математика', 87)

    # Сохраняем данные в файл CSV
    petr.save_to_csv('students.csv')
    kate.save_to_csv('students.csv')
    # Выводим на печать
    print(kate)
    print(petr)
